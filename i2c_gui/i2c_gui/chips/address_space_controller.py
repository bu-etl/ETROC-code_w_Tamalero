...
class Address_Space_Controller(GUI_Helper):
    ...
    def lpgbt(self):
        ...
        if self._rb is not None:
            return self._rb.DAQ_LPGBT
        raise RuntimeError("No LPGBT handle provided to Address_Space_Controller")
    ...
    def _read_memory_address_with_endian(self, address):
        ...
        # --------------------------- NEW ----------------------------
        '''
        tmp = self._i2c_controller.(self._i2c_address, address, read_bytes, self._register_bits, self._register_length, self._read_type)
        '''
    ...
    def write_memory_block(self, address, data_size, write_check: bool = True, read_address = None):
        ...
        for i in range(data_size):
            self.write_memory_register(address+i, write_check=write_check)
        return True
